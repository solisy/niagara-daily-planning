...
def build_policy_adherence(
    fg_inv: pd.DataFrame, forecast: pd.DataFrame, policy: pd.DataFrame, today: str
) -> pd.DataFrame:
    fc7 = forecast[
        (forecast["date"] >= today)
        & (forecast["date"] < (pd.to_datetime(today) + pd.Timedelta(days=7)).date().isoformat())
    ]
    mu = (
        fc7.groupby("sku", as_index=False)["forecast_cases"]
        .mean()
        .rename(columns={"forecast_cases": "avg_daily_fcst_7d"})
    )
    ...
    df["status"] = np.where(
        df["dos"] < df["min_dos"],
        "RED",
        np.where(df["dos"] > df["max_dos"], "YELLOW", "GREEN"),
    )
    df["recommended_prod_cases"] = (
        (df["target_dos"] * df["avg_daily_fcst_7d"] - df["on_hand_cases"])
        .clip(lower=0)
        .round()
        .astype(int)
    )
    ...
    df["recommended_prod_cases"] = (
        np.ceil(df["recommended_prod_cases"] / df["pack_rounding"]) * df["pack_rounding"]
    ).astype(int)
...
def chart_line_load(schedule: pd.DataFrame, out_path: str):
    load = (
        schedule.groupby("line_id", as_index=False)["planned_qty_cases"]
        .sum()
        .sort_values("planned_qty_cases", ascending=False)
    )
    plt.figure()
...
def chart_otif_risk(orders: pd.DataFrame, schedule: pd.DataFrame, out_path: str):
    # Risk proxy: unmet qty by SKU (if any)
    ...
    if unmet.empty:
        plt.text(
            0.5,
            0.5,
            "No unmet cases today\n(schedule feasible under capacity/eligibility)",
            ha="center",
            va="center",
        )
...
def main(data_dir: str, results_dir: str):
    ...
    chart_line_load(
        sched[sched["line_id"] != "UNASSIGNED"],
        os.path.join(results_dir, "charts", "line_load.png"),
    )
    chart_otif_risk(
        orders[orders["order_date"] == today],
        sched,
        os.path.join(results_dir, "charts", "otif_risk.png"),
    )
